...
class CimaLightProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val episodeLinks = episodeAnchors
            .mapNotNull { a ->
                ...
                // accept if text hints episode or if URL is watch.php
                if (t.contains("الحلقة") || eUrl.contains("watch.php?vid=")) {
                    Triple(t, eUrl, t)
                } else null
            }
        ...
        if (episodeLinks.size >= 2) {
            val episodes = episodeLinks.map { (text, eUrl, _) ->
                ...
                // IMPORTANT: your API expects url=..., not data=...
                newEpisode(
                    url = eUrl,
                    name = text.ifBlank { "Episode" },
                    season = 1,
                    episode = epNum
                )
            }.distinctBy { it.url }
    ...
    // ---------------------------
    // LoadLinks (FIXED: gather from play.php AND downloads.php)
    // ---------------------------
    override suspend fun loadLinks(
        ...
        if (downloadsDoc != null) {
            // Grab all anchors first (downloads page usually has lots of mirrors)
